...
def heart_rate_plot(ax, id, start_date, end_date):
    ...
    try:
        ...
        # Load and preprocess the heart rate data
        query_heart_rate = f"""
        SELECT * FROM {table_heart_rate}
        WHERE date BETWEEN '{start_date}' AND '{end_date}'
        """
        df = pd.read_sql(query_heart_rate, db)

        # Load and preprocess the resting heart rate data
        query_resting_heart_rate = f"""
        SELECT * FROM {table_resting_heart_rate}
        WHERE date BETWEEN '{start_date}' AND '{end_date}'
        """
        ds = pd.read_sql(query_resting_heart_rate, db)
        df = df[df['date'] != '-1']
...
def plot_activity(ax, id, start_date, end_date):
    ...
    try:
        ...
        WHERE date BETWEEN '{start_date}' AND '{end_date}'
        """
        df = pd.read_sql(query_activity, db)
...
def sleep_graph_ver(ax, id, start_date, end_date, nan_value_df):
    ...
    try:
        ...
        WHERE date BETWEEN '{start_date}' AND '{end_date}'
        """
        sleep_detail_df = pd.read_sql(query_sleep_detail, db)
